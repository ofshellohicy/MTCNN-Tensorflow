...
sys.path.append("%s/prepare_data" % init_path.ROOT_DIR)
print(sys.path)
...
def train_model(base_lr, loss, data_num):
    ...
    boundaries = [
        int(epoch * data_num / config.BATCH_SIZE) for epoch in config.LR_EPOCH
    ]
    # lr_values[0.01,0.001,0.0001,0.00001]
    lr_values = [
        base_lr * (lr_factor**x) for x in range(0,
...
def train(net_factory, prefix, end_epoch, base_dir, display=200, base_lr=0.01):
    ...
    # RNet use 3 tfrecords to get data
        ...
        landmark_dir = os.path.join(
            '%s/../DATA/imglists/RNet' % init_path.ROOT_DIR,
    ...
    total_loss_op = radio_cls_loss * cls_loss_op + radio_bbox_loss * \
        bbox_loss_op + radio_landmark_loss * landmark_loss_op + L2_loss_op
    ...
    sess = InteractiveSession(config=cf)
    #sess = tf.Session()
    ...
    logs_dir = "%s/logs/%s" % (init_path.ROOT_DIR, net)
        ...
        for step in range(MAX_STEP):
            ...
            if (step + 1) % display == 0:
                ...
                total_loss = radio_cls_loss * cls_loss + radio_bbox_loss * \
                    bbox_loss + radio_landmark_loss * landmark_loss + L2_loss
                # landmark loss: %4f,
                print(
