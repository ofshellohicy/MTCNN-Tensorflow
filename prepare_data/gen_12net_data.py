...
pos_save_dir = "%s/../DATA/12/positive" % init_path.ROOT_DIR

part_save_dir = "%s/../DATA/12/part" % init_path.ROOT_DIR

neg_save_dir = '%s/../DATA/12/negative' % init_path.ROOT_DIR

save_dir = "%s/../DATA/12" % init_path.ROOT_DIR
...
for annotation in annotations:
    ...
    while neg_num < 50:
        ...
        if np.max(Iou) < 0.3:
            ...
            f2.write("%s/../DATA/12/negative/%s.jpg" %
                     (init_path.ROOT_DIR, n_idx) + ' 0\n')
    ...
    for box in boxes:
        ...
        for i in range(20):
            ...
            if iou >= 0.65:
                ...
                f1.write("%s/../DATA/12/positive/%s.jpg" %
                         (init_path.ROOT_DIR, p_idx) +
                         ' 1 %.2f %.2f %.2f %.2f\n' %
                ...
                f3.write("%s/../DATA/12/part/%s.jpg" %
                         (init_path.ROOT_DIR, d_idx) +
                         ' -1 %.2f %.2f %.2f %.2f\n' %
                         (offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
