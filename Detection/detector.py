...
class Detector(object):
    ...
    def __init__(self, net_factory, data_size, batch_size, model_path):
        ...
        with graph.as_default():
            self.image_op = tf.compat.v1.placeholder(
                tf.float32,
                shape=[batch_size, data_size, data_size, 3],
                name='input_image')
            #figure out landmark
            self.cls_prob, self.bbox_pred, self.landmark_pred = net_factory(
                self.image_op, training=False)
    ...
    def predict(self, databatch):
        ...
        return np.concatenate(cls_prob_list, axis=0), np.concatenate(
            bbox_pred_list, axis=0), np.concatenate(landmark_pred_list, axis=0)
